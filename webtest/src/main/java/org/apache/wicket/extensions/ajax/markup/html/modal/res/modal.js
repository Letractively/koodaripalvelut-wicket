...
if (typeof(Wicket.Object) == "undefined") {
	Wicket.Object = { };
}

if (typeof(Wicket.Object.extend) == "undefined") {
...
Wicket.Iframe = {
	...
	documentFix: function(doc, revertList) {
		...
		for (var i = 0; i < iframes.length; ++i) {
			...
			if (typeof(iframe.tagName) != "undefined" && iframe.tagName != null)
...
/**
 * Keep a stack of displayed windows.
 */
if (typeof(Wicket.Window.windowStack) == "undefined") {
  try {
    Wicket.Window.windowStack = window.parent.Wicket.Window.windowStack;
  } catch (ignore) {}
  if (typeof(Wicket.Window.windowStack) == "undefined") {
    Wicket.Window.windowStack = new Array();
  }
}

/**
 * Keep a map of windows by their id.
 */
if (typeof(Wicket.Window.windowMap) == "undefined") {
  try {
    Wicket.Window.windowMap = window.parent.Wicket.Window.windowMap;
  } catch (ignore) {}
  if (typeof(Wicket.Window.windowMap) == "undefined") {
    Wicket.Window.windowMap = {};
  }
}

/**
 * Asssign the baseWindow from which point modals will show up.
 */
try {
  Wicket.Window.baseWindow == window.parent.Wicket.Window.baseWindow;
} catch (ignore) {}
if (typeof(Wicket.Window.baseWindow) == "undefined") {
  Wicket.Window.baseWindow = window.parent;
}
if (typeof(Wicket.Window.baseWindow) == "undefined") {
  Wicket.Window.baseWindow = window;
}
...
Wicket.Window.create = function(settings) {
	...
	return new Wicket.Window.baseWindow.Wicket.Window(settings);
}
...
Wicket.Window.get = function() {
	...
	var wi = Wicket.Window.windowStack.length - 1;
	if (wi >= 0)
	  win = Wicket.Window.windowStack[wi];
	return win;
}
...
Wicket.Window.close = function() {
  ...
  if (wi >= 0)
  {
    var win = Wicket.Window.windowStack[wi];
...
Wicket.Window.prototype = {
	...
	createDOM: function() {
		...
		Wicket.Window.windowMap[this.settings.id] = this;
	},
	...
	adjustOpenWindowZIndexesOnShow: function() {
		...
		if (typeof(this.oldWindow) != "undefined" && this.oldWindow != null) {
	...
	adjustOpenWindowsStatusOnShow: function() {
	  var wi = Wicket.Window.windowStack.length - 1;
		...
		if (wi >= 0) {
			...
			this.oldWindow = Wicket.Window.windowStack[wi];
		}
		// keep reference to this window
		Wicket.Window.windowStack.push(this);
	},
	...
	canCloseInternal: function() {
	  for (var wi = Wicket.Window.windowStack.length - 2; wi >= 0; wi--) {
	    var w = Wicket.Window.windowStack[wi];
	    if (this === w) {
	...
	close: function(force) {
		...
		// clear window from the window map.
		try { delete Wicket.Window.windowMap[this.window.id]; }
		catch (e) { Wicket.Window.windowMap[this.window.id] = null; }
		...
		var ws = Wicket.Window.windowStack.length;
		for (var wi = 0; wi < ws; wi++) {
      var w = Wicket.Window.windowStack.shift();
      if (this !== w) {
        Wicket.Window.windowStack.push(w);
      }
    }
	},
...
Wicket.Window.Mask.prototype = {
	...
	disableCoveredContent: function() {
		var doc = document;
		var old;
		var wi = Wicket.Window.windowStack.length - 2;
    // is there a window displayed already?
    if (wi >= 0) {
      // save the reference to it
      old = Wicket.Window.windowStack[wi];
    }
		if (typeof(old) != "undefined" && old != null) {
			doc = old.getContentDocument();
		}

		this.doDisable(doc, Wicket.Window.windowStack[wi + 1]);
	},
