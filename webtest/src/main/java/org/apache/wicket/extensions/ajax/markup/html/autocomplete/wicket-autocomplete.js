...
Wicket.AutoComplete=function(elementId, callbackUrl, cfg, indicatorId){
    ...
    function updateChoices(showAll){
    	...
    	var word = wicketGet(elementId).value;
    	
     	if (word.length > 0) {
		   	do {
		   		if (dict[word] != undefined && cacheValid(dict[word])) {
		   			if (dict[word].resp === false){
		   				doUpdateChoices("", dict[word]);
		   				return;
		   			} 
		   			break;
		   		}
    ...
    function cacheValid(cacheExp){
    	var tm = new Date().getTime();
   	    if (cfg.cacheDuration == 0 || tm < cacheExp.tm)
    		return true;
    	return false;
    }
    ...
    function actualUpdateChoices()
    {
        ...
        var word = wicketGet(elementId).value;
        if (dict[word] != undefined && cacheValid(dict[word])) {
        	doUpdateChoices(dict[word].resp, dict[word]);
        } else {
       	  var request = new Wicket.Ajax.Request(callbackUrl+(callbackUrl.indexOf("?")>-1 ? "&" : "?") + "q="+processValue(word), doUpdateChoices, false, true, false, "wicket-autocomplete|d");
       	  request.get();
        }
    }
    ...
    function doUpdateChoices(resp, cacheElem) {
    	//make sure cache timeout is not reset
   		var tm = new Date().getTime() + cfg.cacheDuration;
        ...
        if (cfg.cacheDuration >= 0) {
	        if (!(element && element.firstChild && element.firstChild.children) || element.firstChild.children.length == 0){
	        	dict[word] = {tm : tm, resp: false};
	        } else {
	        	dict[word] = {tm : tm, resp: resp};
	        }
        }
