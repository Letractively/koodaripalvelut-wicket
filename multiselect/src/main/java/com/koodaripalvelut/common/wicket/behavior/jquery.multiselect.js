...
$.widget("ech.multiselect", {
	...
	_create: function(){
		...
		el.find('option').each(function(i){
			...
			if(true || value.length > 0 ){
				...
				if (o.multiple) {
          html.push('<input id="'+inputID+i+'" type="radio" class="ui-multiselect-multi-radio" value="'+inputID+'" title="'+title+'"/>&nbsp;');
        }
	...
	_bindEvents: function(){
		...
		.delegate(':checkbox, :radio', 'click', function(e){
			var $this = $(this),
				...
				if (self.options.multiple && $this.is(':radio')) {
				    self.menu.find(':checkbox').removeAttr('checked');
				    self.radios.not(this).removeAttr('checked');
            self.menu.find("'#"+val+"'").attr('checked', 'true');
            val = $(self.menu.find("'#"+val+"'")[0]).val();
            e.preventDefault();
				}

			// bail if this input is disabled or the event is cancelled
			if( $this.is(':disabled') || self._trigger('click', e, { value:val, text:this.title, checked:checked, radio:$this.is(':radio') }) === false ){
				e.preventDefault();
				return;
			}
	...
	_setButtonWidth: function(){
		var width = this.element[0].style.width,
			o = this.options;
...
//      alert("this.element=" + elsu[0].tagName + " outerWidth: " + width + " vs. width: " + elsu[0].style.width);
	...
	update: function( offset ){
		var o = this.options,
			...
			$checked = $inputs.filter(':checked').not(o.multiple ? ':radio' : ':checkbox'),
